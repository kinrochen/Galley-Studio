...
for (const required of ["main.js", "release/galley-studio-0.2.3.zip"]) {
  try {
...
try {
  const archive = unzipSync(new Uint8Array(await readFile("release/galley-studio-0.2.3.zip")));
  for (const [path, bytes] of Object.entries(archive)) {
    inspect(`release:${path}`, new TextDecoder().decode(bytes));
  }
} catch (error) {
  if (error?.code !== "ENOENT") failures.push(`release/galley-studio-0.2.3.zip: invalid ZIP (${error instanceof Error ? error.message : String(error)})`);
}

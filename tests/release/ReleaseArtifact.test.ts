...
it("contains exactly the five 0.2.4 release files when the release gate has run", () => {
  const path = resolve("release/galley-studio-0.2.4.zip");
  if (!existsSync(path)) {
  ...
  expect(main).not.toMatch(/import\(\s*["'`]node:/u);
